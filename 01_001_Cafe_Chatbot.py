...
class Chatbot():
    def __init__(self):
        super().__init__()
        self.token = "5529723218:AAFYQrjBOpiffer7nDtLDj0NWsVGA0Hqoo0"
        self.id = '5582037267'
        self.device = torch.device('cpu')
        self.bertmodel, self.vocab = get_pytorch_kobert_model()
        self.model = BERTClassifier(self.bertmodel, dr_rate=0.5)
        self.model.load_state_dict(torch.load('G:\\내 드라이브\\Watching_You_PJ\\project_ChatBot\\03_DL_Model\\01_001_bert_10.pth',map_location=self.device))
        ...
        self.df = pd.read_csv('G:\\내 드라이브\\Watching_You_PJ\\project_ChatBot\\02_preprocessing\\01_001_bert.csv')
...
더 필요하신 거 없으세요?
    ...
    def input_category(self,input_data):
        df = pd.read_csv('G:\\내 드라이브\\Watching_You_PJ\\project_ChatBot\\01_DataSet\\1_Raw_Data\\input_label.csv')
        input_len = len(df['C'])
